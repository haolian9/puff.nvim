...
local next_canvas_id
do
  local count = 0
  function next_canvas_id()
    count = count + 1
    return count
  end
end
...
local function main(entries, formatter, prompt, callback)
  local lines = {}
  do
  ...
  do -- setup buf
    ...
    bufrename(canvas.bufnr, string.format("tui://menu#%d", next_canvas_id()))
...
return function(entries, opts, callback)
  ...
  main(entries, formatter, prompt, callback)
end
